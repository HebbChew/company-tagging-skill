...
para(doc, '区域生物医药企业全景标注分析报告', '黑体', 24, True, WD_ALIGN_PARAGRAPH.CENTER, indent=False)
doc.add_paragraph()
...
add_table(doc,
    ['指标', '家数', '占比/说明'],
    [['生物医药核心主业（R0）', '1398', '52%'],
     ['泛健康（R1）', '109', '有健康监管资质的消费健康/合成生物非医药应用'],
     ['供应链与产业配套（R2）', '162', '含医药供应链上游配套（阀门/气体/装备耗材）'],
     ['大健康边缘（R3）', '117', '无资质美妆/食品营销、养生等，建议降级观察'],
     ['完全无关（R4）', '37', '建议移出名单'],
     ['完成搜索核验', '1829', '68%；其中 979 家获得带证据的行业地位表述'],
     ['识别出母公司', '503', '36 个集团在区域内多主体布局'],
...
add_table(doc, ['相关度', '家数', '含义', '名单建议'],
    [['R0 核心', '1398', '药/械/IVD/CXO/流通/医疗服务', '名单主体'],
     ['R1 泛健康', '109', '有健康资质的消费品/合成生物非医药应用', '保留标注'],
     ['R2 配套', '162', '供应链上游+产业服务平台', '单列统计'],
     ['R3 边缘', '117', '无资质美妆/食品营销、养生', '降级观察'],
     ['R4 无关', '37', '纺织检测/宠物殡葬/个护电器等', '建议移出']],
...
para(doc, '一级体系经融合收束为 10 个（原 12 个：数据与自动化/模型系统与平台/医药研究支撑等已按内容归并，生物制造与合成生物合并为“合成生物与生物制造”，制剂与递送统一归入药物体系）。医药供应链与流通（319 家）与医疗器械（317 家）构成名单底盘；药物（156 家）与 CXO（140 家）为中坚；合成生物、脑机接口、核酸药物等前沿方向在小微段密集出现。')
h2(doc, '（一）细分赛道 TOP10 与代表企业')
add_table(doc, ['细分赛道', '家数', '代表企业（地位均经核验）'],
    [['医药流通与供应链', '264', '生生医药冷链（温控供应链全国第一）、朗脉洁净（国家级小巨人）'],
     ['IVD-平台技术', '71', '思路迪（全球首款外泌体卵巢癌 IVD）、之江生物（688317）'],
     ['CRO-发现与前期研究', '65', '中科新生命（中科院质谱多组学）、欧易生物（嘉必优收购）'],
     ['医用材料与耗材', '59', '士卓曼（全球牙科种植体龙头）、上药第一生化'],
     ['医药供应链上游配套', '55', '东富龙（制药装备龙头）、盖米阀门（GEMÜ）'],
     ['康复与家用医疗', '49', '互邦智能（轮椅国标起草单位）'],
     ['小分子药物', '47', '汇伦医药（2026 递表港交所）'],
     ['介入与植入', '43', '捍宇医疗（国产首款）、励楷科技（D 轮近 5 亿）'],
     ['细胞与基因治疗', '37', '华夏源、健信生物'],
     ['影像设备', '22', '澳华内镜（688212）、美沃精密']],
    widths=[4.0, 1.6, 9.6])
...
# ===== 四、集团化布局 =====
h1(doc, '四、集团化布局：481 家有母公司，36 个集团多主体落地')
add_table(doc, ['类型', '代表'],
...
add_table(doc, ['注册资本档', '家数', '已搜索', '有地位词', '有母公司', '无信息'],
    [['≥5000 万', '152', '100%', '88%', '57%', '1%'],
     ['2000-5000 万', '141', '100%', '80%', '48%', '6%'],
...
# ===== 六、名单质量与时效 =====
h1(doc, '六、名单质量与时效警示')
para(doc, '**（一）边界企业 154 家**（清单单独成表）：R4 建议移出 37 家（纺织品检测×2、宠物殡葬、水族电商、家庭园艺肥料、指纹考勤机等）；R3 降级观察 117 家（美妆营销 62、食品营销 36、养生 2、其他 13）。')
para(doc, '**（二）无公开信息 668 家**：特征为整百万注册资本聚集（462 家恰好 100 万）、集中登记地、2025-2026 批量新设。建议与招商口径对齐决定去留。')
...
para(doc, '已知残留风险（如实披露）：「登记」档流通标签约三分之二准确（工商登记口径未经实证）；197 家存疑待核母公司建议天眼查穿透复核；本次核验基于搜索摘要级证据，不等于全文尽调。')

# ===== 八、建议 =====
h1(doc, '八、建议动作')
para(doc, '（一）名单管理：R4 移出、R3 降级观察、671 家无信息企业按招商口径决定去留。')
para(doc, '（二）招商跟进：第三节亮点企业（脑机/CGT/AI 制药/器械创新）建议纳入重点项目跟踪表。')
para(doc, '（三）数据资产：197 家存疑母公司走天眼查批量穿透；每季度对已核验企业跑一次时效核验（分拆/更名/获批）；无信息清单作为招商摸排任务来源。')
doc.add_paragraph()
